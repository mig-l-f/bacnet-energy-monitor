...
/* returns network byte order */
uint8_t* bip_get_broadcast_addr(
    void)
{
...
//        return 0;
    if(getRXReceivedSize_func(CW5100Class_new(), BIP_Socket)){
    	...
    	received_bytes = (int)recvfrom_func(BIP_Socket, &pdu[0], max_pdu, src_addr, &src_port);
    }
    ...
    if ((function == BVLC_ORIGINAL_UNICAST_NPDU) ||
        ...
        if ((convertBIP_Address2uint32(src_addr) == convertBIP_Address2uint32(BIP_Address)) &&
            (src_port == BIP_Port)) {
            pdu_len = 0;
#if 0
