...
bool DeviceObject::getValid_Object_Instance_Number(uint32_t object_id) {
	return ((Object_Identifier.instance == object_id) || (object_id == BACNET_MAX_INSTANCE));
}
bool DeviceObject::isWithinLimits(int32_t& low_limit, int32_t& high_limit){
	return ((Object_Identifier.instance >= (uint32_t)low_limit) & (Object_Identifier.instance <= (uint32_t)high_limit));
}

int DeviceObject::Object_Read_Property(BACNET_READ_PROPERTY_DATA * rpdata) {

		int16_t apdu_len = 0;   /* return value */
	    int16_t len = 0;        /* apdu len intermediate value */
	    uint8_t *apdu = NULL;
	    ...
	    switch (rpdata->object_property) {
	    	case PROP_OBJECT_IDENTIFIER:
	    		apdu_len = (int16_t)encode_application_object_id(&apdu[0], Object_Identifier.type, Object_Identifier.instance);
	    		break;
	    	case PROP_OBJECT_NAME:
	    		apdu_len = (int16_t)encode_application_character_string(&apdu[0], &Object_Name);
	    		break;
	    	case PROP_OBJECT_TYPE:
	    		apdu_len = (int16_t)encode_application_enumerated(&apdu[0], Object_Identifier.type);
	    		break;
	    	case PROP_SYSTEM_STATUS:
	    		apdu_len = (int16_t)encode_application_enumerated(&apdu[0], System_Status);
	    		break;
	    	case PROP_VENDOR_NAME:
	    		apdu_len = (int16_t)encode_application_character_string(&apdu[0], &Vendor_Name);
	    		break;
	    	case PROP_VENDOR_IDENTIFIER:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], Vendor_Identifier);
	    		break;
	    	case PROP_MODEL_NAME:
	    		apdu_len = (int16_t)encode_application_character_string(&apdu[0], &Model_Name);
	    		break;
	    	case PROP_FIRMWARE_REVISION:
	    		apdu_len = (int16_t)encode_application_character_string(&apdu[0], &Firmware_Revision);
	    		break;
	    	case PROP_APPLICATION_SOFTWARE_VERSION:
	    		apdu_len = (int16_t)encode_application_character_string(&apdu[0], &Application_Software_Version);
	    		break;
	    	case PROP_PROTOCOL_VERSION:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], BACNET_PROTOCOL_VERSION);
	    		break;
	    	case PROP_PROTOCOL_REVISION:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], BACNET_PROTOCOL_REVISION);
	    		break;
	    	case PROP_PROTOCOL_SERVICES_SUPPORTED:
	    		...
	    		apdu_len = (int16_t)encode_application_bitstring(&apdu[0], &bit_string);
	    		break;
	    	case PROP_PROTOCOL_OBJECT_TYPES_SUPPORTED:
	    		bitstring_init(&Protocol_Object_Types_Supported);
				for (uint8_t i = 0; i < MAX_ASHRAE_OBJECT_TYPE; i++) {
					bitstring_set_bit(&Protocol_Object_Types_Supported, (uint8_t) i, false);
				}
				for (uint8_t i = 0; i < numberOfObjectsInList; i++) {
					bitstring_set_bit(&Protocol_Object_Types_Supported, Object_List[i].type, true);
				}
	    		apdu_len = (int16_t)encode_application_bitstring(&apdu[0], &Protocol_Object_Types_Supported);
	    		break;
	    	case PROP_OBJECT_LIST:
	    		if (rpdata->array_index == 0) {// Return Count of Objects
	    			apdu_len = (int16_t)encode_application_unsigned(&apdu[0], MAX_BACNET_OBJECTS_PER_DEVICE);

	    		}else if (rpdata->array_index == BACNET_ARRAY_ALL){ // Return ID of all objects
	    			for (uint8_t i = 0; i < MAX_BACNET_OBJECTS_PER_DEVICE; i++) {

	    				len = (int16_t)encode_application_object_id(&apdu[apdu_len],
	    			...
	    			if (rpdata->array_index <= MAX_BACNET_OBJECTS_PER_DEVICE) {
	    				...
	    				apdu_len = (int16_t)encode_application_object_id(&apdu[0],
	    	...
	    	case PROP_MAX_APDU_LENGTH_ACCEPTED:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], Max_APDU_Length_Accepted);
	    		break;
	    	case PROP_SEGMENTATION_SUPPORTED:
	    		apdu_len = (int16_t)encode_application_enumerated(&apdu[0], Segmentation_Supported);
	    		break;
	    	case PROP_APDU_TIMEOUT:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], APDU_Timeout);
	    		break;
	    	case PROP_NUMBER_OF_APDU_RETRIES:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], 3);
	    		break;
	    	case PROP_DEVICE_ADDRESS_BINDING:
	    		//apdu_len = address_list_encode(&apdu[0], MAX_APDU);
	    		break;
	    	case PROP_DATABASE_REVISION:
	    		apdu_len = (int16_t)encode_application_unsigned(&apdu[0], Database_Revision);
	    		break;
	    	default:
	    		rpdata->error_class = ERROR_CLASS_PROPERTY;
