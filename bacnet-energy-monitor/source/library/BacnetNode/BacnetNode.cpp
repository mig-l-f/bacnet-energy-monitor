...
void BacnetNode::dealWithErrorCodes(int16_t & len, int16_t& npdu_len,
	...
	switch(len){
		case BACNET_STATUS_REJECT:
			apdu_len = (int16_t)reject_encode_apdu(&Handler_Transmit_Buffer[npdu_len],
		...
		case BACNET_STATUS_ABORT: //Not Covered by Unit Tests
			apdu_len = (int16_t)abort_encode_apdu(&Handler_Transmit_Buffer[npdu_len],
		...
		case BACNET_STATUS_ERROR: //Not Covered by Unit Tests
			apdu_len = (int16_t)bacerror_encode_apdu(&Handler_Transmit_Buffer[npdu_len],
			        service_data->invoke_id, SERVICE_CONFIRMED_READ_PROPERTY,
			        rpdata.error_class, rpdata.error_code);
			VERBOSE_STRING(PSTR("Sending error status.\n"));
			break;
	}
	int16_t pdu_len = npdu_len + apdu_len;
...
void BacnetNode::handler_who_is(uint8_t * service_request, uint16_t service_len,
    ...
    len = (int16_t) whois_decode_service_request(service_request, service_len, &low_limit, &high_limit);
...
//    		device->getObjectIdentifier()->instance, low_limit, high_limit);
    ...
    if (len == 0){
    	...
    	if (device->isWithinLimits(low_limit, high_limit)){
...
void BacnetNode::sendIamUnicast(uint8_t * buffer, BACNET_ADDRESS * src){
    ...
    npdu_len = (int16_t)npdu_encode_pdu(&buffer[0], &dest, &my_address, &npdu_data);
    /* encode the APDU portion of the packet */
    apdu_len = (int16_t)iam_encode_apdu(&buffer[npdu_len],
